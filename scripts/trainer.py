...
def train(config, model, train_dl, val_dl):
    ...
    for epoch in range(1, config.epochs+1):
        train_loss = train_epoch(config, model, config.device, train_dl, epoch, optimizer, scaler)
        val_loss = val_epoch(model, config.device, val_dl, epoch)
        wandb.log({"loss/train_epoch": train_loss, "loss/val_epoch": val_loss, "epoch":epoch})