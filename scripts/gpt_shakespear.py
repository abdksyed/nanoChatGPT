...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(
        self, emb_dim: int, head_size: int, seq_len: int, num_heads: int, dropout
    ):
        super().__init__()
        self.heads = nn.ModuleList(
            [Head(emb_dim, head_size, seq_len, dropout) for _ in range(num_heads)]
        )
...
class Block(nn.Module):
    ...
    def __init__(self, emb_dim: int, seq_len: int, num_heads: int, dropout: float):
        ...
        self.sa_head = MultiHeadAttention(
            emb_dim, head_size, seq_len, num_heads, dropout
        )
...
class GPT(nn.Module):
    ...
    def generate(self, idx, max_new_tokens):
        ...
        for _ in range(max_new_tokens):
            ...
            idx_cropped = idx[:, -self.seq_len :]  # (batch, seq_len)
